...
export default class Test extends Component<any> {
  ...
  auxOpenCellNarrow = (param: AuxOpenCellParam): AuxOpenCellParam => {
    ...
    return nextCoords.reduce(
      (acc, nextCoord) => this.auxOpenCellNarrow({ ...acc, target: nextCoord }),
      nextParam
    );
  };
  ...
  auxOpenCellWide = (param: AuxOpenCellParam): AuxOpenCellParam => {
    if (
      isArrIncludesCoord(param.antiAcc, param.target) ||
      isArrIncludesCoord(param.acc, param.target)
    ) {
      return param;
    }
